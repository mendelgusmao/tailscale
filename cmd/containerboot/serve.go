...
func readSimpleServeConfig(configs []string, certDomain string) (*ipn.ServeConfig, error) {
	const (
		...
		invalidTargetProtoMessage = "simpleserve: invalid target proto in serve config: %s"
		invalidTargetPortMessage  = "simpleserve: invalid target port in serve config: %s"
	...
	for _, config := range configs {
		...
		if !slices.Contains(allowedProtos, uri.Scheme) {
			return nil, fmt.Errorf(invalidTargetProtoMessage, target)
		}
		...
		default:
			return nil, fmt.Errorf("invalid proto in serve config: %s", proto)
		}
